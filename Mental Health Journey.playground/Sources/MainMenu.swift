...
public class MainMenu: SKScene {
    ...
    override public func didMove(to view: SKView) {
        ...
        loadSweetFont()
        loadBalsamiqFont()
        loadBalsamiqFontBold()
        
        self.run(SKAction.playSoundFileNamed("Intro/teamwork.mp3", waitForCompletion: false))
    }
    ...
    func loadBalsamiqFontBold(){
        let cfURL = Bundle.main.url(forResource: "Fonts/BalsamiqSans-Bold", withExtension: "ttf")! as CFURL
        CTFontManagerRegisterFontsForURL(cfURL, CTFontManagerScope.process, nil)
        var fontNames: [[AnyObject]] = []
        for name in UIFont.familyNames {
           fontNames.append(UIFont.fontNames(forFamilyName: name) as [AnyObject])
